...
export default function TShirtPage() {
  ...
  useEffect(() => {
    ...
    tShirtApi
      ...
      .catch(() => setError('Could not load t-shirt products right now.'))
    ...
    <section className="min-h-screen bg-gradient-to-b from-orange-50 via-white to-white">
      <div className="relative overflow-hidden">
        ...
        <div className="relative mx-auto max-w-7xl px-6 py-16 text-center text-white lg:px-8">
          ...
          <h1 className="mt-5 text-4xl font-bold lg:text-5xl">Custom Printed T-Shirts</h1>
          <p className="mx-auto mt-4 max-w-2xl text-white/90">
            Upload your logo, pick sizes, add print notes — same easy checkout as our other products.
          </p>
        </div>
      </div>
        ...
        {!loading && !error && (
          <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3">
            {products.map((product) => (
              <TShirtProductCard key={product._id} product={product} />
            ))}
          </div>
        )}

        {!loading && !error && products.length === 0 && (
          <p className="text-center text-slate-500">No t-shirt products available yet. Please check back soon.</p>
        )}
      </div>
    </section>
  )
