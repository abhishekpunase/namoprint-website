...
export default function CorporateGiftPage() {
  ...
  useEffect(() => {
    ...
    corporateGiftApi
      ...
      .catch(() => setError('Could not load corporate gift products right now.'))
      .finally(() => setLoading(false))
  }, [])

  return (
    <section className="min-h-screen bg-gradient-to-b from-slate-50 via-white to-white">
      <div className="relative overflow-hidden">
        ...
        <div className="relative mx-auto max-w-7xl px-6 py-16 text-center text-white lg:px-8">
          ...
          <h1 className="mt-5 text-4xl font-bold lg:text-5xl">Branded Corporate Gifts</h1>
          <p className="mx-auto mt-4 max-w-2xl text-white/90">
            Upload your logo or design file — we print, pack, and deliver for your team, clients, and events.
          </p>
        </div>
      </div>
        ...
        {!loading && !error && (
          <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3">
            {products.map((product) => (
              <CorporateGiftProductCard key={product._id} product={product} />
            ))}
          </div>
        )}

        {!loading && !error && products.length === 0 && (
          <p className="text-center text-slate-500">No corporate gift products available yet.</p>
        )}
      </div>
    </section>
  )
