...
export function AdminBabyBirthFrameProductsPage() {
  ...
  const submit = async (event) => {
    ...
    try {
      ...
      const payload = {
        ...
        images: form.images,
        qualityOptions,
        maxPhotos: Number(form.maxPhotos || 3),
      ...
      if (editingId) {
        await babyBirthFrameApi.adminUpdate(editingId, payload)
        setMessage('Baby birth frame product updated')
      } else {
        await babyBirthFrameApi.adminCreate(payload)
        setMessage('Baby birth frame product created')
      }
      resetForm()
      loadProducts()
    } catch (err) {
  ...
  const startEdit = (product) => {
    ...
    setForm({
      title: product.title || '',
      subtitle: product.subtitle || 'A Memory for Life',
      ...
      maxPhotos: String(product.maxPhotos ?? 3),
      genderOptions: (product.genderOptions || ['Boy', 'Girl']).join(', '),
  ...
  const remove = async (id) => {
    if (!window.confirm('Deactivate this baby birth frame product?')) return
    try {
      await babyBirthFrameApi.adminDelete(id)
      loadProducts()
    } catch (err) {
      setError(err.message)
    }
  ...
  return (
    <div className="admin-page">
      <div className="admin-page-head">
        <div>
          <p className="eyebrow">Baby Birth Frame Catalog</p>
          <h1>Baby Birth Frames</h1>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 gap-5">
        <section className="admin-panel">
          ...
          <form className="admin-form sm:grid-cols-2" onSubmit={submit}>
            ...
            <label>
              ...
              <input
                placeholder="Premium finish, Custom birth details, Gift packaging"
            ...
            <label>
              Gender options (comma separated)
              <input
                value={form.genderOptions}
                onChange={(e) => setForm({ ...form, genderOptions: e.target.value })}
              />
            </label>
            ...
            <div className="admin-options-field sm:col-span-2">
              ...
              {form.qualityOptions.map((opt, index) => (
                <div className="admin-option-row" key={opt._id || index}>
                  <input
                    placeholder="Label e.g. 8x10 inch Standard"
                    value={opt.label}
            ...
            <div className="admin-seo-section sm:col-span-2">
              <h3>SEO</h3>
              <ProductSeoFields form={form} setForm={setForm} urlPrefix="/baby-birth-frames/" slug={form.slug} />
            </div>
        ...
        <section className="admin-panel">
          <h2>All baby birth frames ({products.length})</h2>
          <div className="admin-table-wrap">
            <table className="admin-table">
              ...
              <tbody>
                ...
                {!products.length && (
                  <tr>
                    <td colSpan={5}>No baby birth frame products yet. Add one from the form.</td>
                  </tr>
                )}
              </tbody>
            </table>
