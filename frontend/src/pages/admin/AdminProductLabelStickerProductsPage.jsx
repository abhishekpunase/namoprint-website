...
const emptyForm = {
  ...
  labelUploadHint: 'Upload label image in the exact size you selected',
  isFeatured: false,
  isActive: true,
  sortOrder: '0',
  slug: '',
...
export function AdminProductLabelStickerProductsPage() {
  ...
  const submit = async (event) => {
    ...
    try {
      ...
      const payload = {
        ...
        images: form.images,
        qualityOptions,
        labelUploadHint: form.labelUploadHint,
      ...
      if (editingId) {
        await productLabelStickerApi.adminUpdate(editingId, payload)
        setMessage('Product label sticker product updated')
      } else {
        await productLabelStickerApi.adminCreate(payload)
        setMessage('Product label sticker product created')
      }
      resetForm()
      loadProducts()
    } catch (err) {
  ...
  const startEdit = (product) => {
    ...
    setForm({
      ...
      labelUploadHint: product.labelUploadHint || 'Upload label image in the exact size you selected',
  ...
  const remove = async (id) => {
    if (!window.confirm('Deactivate this product label sticker product?')) return
    try {
      await productLabelStickerApi.adminDelete(id)
      loadProducts()
    } catch (err) {
      setError(err.message)
    }
  ...
  return (
    <div className="admin-page">
      <div className="admin-page-head">
        <div>
          <p className="eyebrow">Product Label Sticker Catalog</p>
          <h1>Product Label Stickers</h1>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 gap-5">
        <section className="admin-panel">
          ...
          <form className="admin-form sm:grid-cols-2" onSubmit={submit}>
            ...
            <label>
              Label upload hint
              <input
                value={form.labelUploadHint}
                onChange={(e) => setForm({ ...form, labelUploadHint: e.target.value })}
              />
            </label>
            ...
            <label className="sm:col-span-2">
              ...
              <input
                placeholder="Exact size print, Peel & stick, Food safe"
            ...
            <div className="admin-options-field sm:col-span-2">
              ...
              {form.qualityOptions.map((opt, index) => (
                <div className="admin-option-row" key={opt._id || index}>
                  <input
                    placeholder="Label e.g. 2×2 inch (50×50 mm)"
                    value={opt.label}
            ...
            <div className="admin-seo-section sm:col-span-2">
              <h3>SEO</h3>
              <ProductSeoFields form={form} setForm={setForm} urlPrefix="/product-label-stickers/" slug={form.slug} />
            </div>
        ...
        <section className="admin-panel">
          <h2>All product label sticker products ({products.length})</h2>
