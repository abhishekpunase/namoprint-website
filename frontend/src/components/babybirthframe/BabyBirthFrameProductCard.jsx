...
export function BabyBirthFrameProductCard({ product }) {
  ...
  const detailPath = `/baby-birth-frames/${product.slug}`
  ...
  return (
    <article className="group relative flex h-full flex-col overflow-hidden rounded-xl border border-gray-200 bg-white transition hover:border-amber-400 hover:shadow-md sm:rounded-2xl sm:border-orange-100 sm:shadow-md sm:hover:-translate-y-1 sm:hover:shadow-xl">
      <div className="absolute inset-x-0 top-0 z-10 hidden h-1 bg-gradient-to-r from-sky-400 via-pink-300 to-orange-300 sm:block" />

      <Link to={detailPath} className="relative block shrink-0 overflow-hidden bg-white sm:bg-sky-50">
        ...
        {mainSrc ? (
          ...
          <div className="flex aspect-square w-full items-center justify-center bg-sky-50 text-sm text-neutral-500 sm:aspect-[4/3]">
            No image
          </div>
        )}
      </Link>
      ...
      <div className="hidden flex-1 flex-col gap-3 p-4 sm:flex">
        <div className="flex items-center justify-between gap-2">
          <span className="truncate rounded-full bg-sky-100 px-3 py-0.5 text-[11px] font-semibold uppercase tracking-wide text-sky-800">
            Baby Birth Frame
          </span>
